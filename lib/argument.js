        ...
        if (this.prompting.choices) {
          ...
          for (var item of this.prompting.choices) {
            if (item.key) {
              ...
              choiceDict[item.display] = item.key;
            } else {
              choice.push(item);
              choiceDict[item] = item;
            }
          }
          this.prompting.choices = choice;
          answer = await this.generator.prompt(_.assignIn({ name: this.name }, this.prompting, { message }));
          const val = choiceDict[answer[this.name]];
          if (val) {
            answer[this.name] = val;
          }
        } else {
          answer = await this.generator.prompt(_.assignIn({ name: this.name }, this.prompting, { message }));
        }
